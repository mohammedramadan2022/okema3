        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($owners)
                ->addColumn('action', function ($owner) {
                    ...
                    <button class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm delete" data-id="' . $owner->id . '">
                        <span class="svg-icon svg-icon-3">
                ...
                ->editColumn('image', function ($owner) {
                    return '
                    <a data-fancybox="" href="' . get_file($owner->image) . '">
                        <img height="60px" src="' . get_file($owner->image) . '">
                    </a>
                ';
                })
